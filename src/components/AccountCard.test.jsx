    ...
    render(
      <UserProvider user={{ name: '김연지', status: '정상' }}>
        <AccountCard
          ...
          showFullNo={false}
          showAmount={false}
          onDeposit={vi.fn()} //key는 안보여서 보이는 값들만 확인하는 작업임
        />
      </UserProvider>
    )
    // Then: 렌더링된 결과물에 마스킹 처리된 텍스트가 존재하는지 검증
    expect(screen.getByText('1002-345-6****1')).toBeInTheDocument() //있나?
     // Then: 렌더링된 결과물에 마스킹이 되지 않은 텍스트가 존재하지 않는지 검증
    expect(screen.queryByText('1002-345-678901')).not.toBeInTheDocument() //없니?
  })
})